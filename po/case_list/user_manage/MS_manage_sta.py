...
class Ticket_Query(myunit.MyTest):
    ...
    def test_show_num_verify(self):
        ...
        self.assertEqual(msg.strip(), '322')
    ...
    #     '''进入移动台管理页面,点击开户,进入开户界面只填写基本信息,填写未注册的正确基本信息'''
    #     self.user_login(username="admin", password="admin")
    ...
    def test_add_icon_verify_1_1(self):
        '''进入移动台管理页面 点击开户 进入开户界面添加一个用户，填写错误的区号（区号最小值减一：327）'''
        self.user_login(username="admin", password="admin")
    ...
    def test_user_map_show_num_2_7(self):
        '''进入移动台管理页面 点击用户映射 进入用户映射页面  配置一条新的用户号码映射范围 错误的队号 队号为最大值加一：90 328 90 200/210  328 90 300/310'''
        self.user_login(username="admin", password="admin")
