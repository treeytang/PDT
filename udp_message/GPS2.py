...
pattern_list = [
    [
        [0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],
        [0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],
        [0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],
        [0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],
        [0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],[0.001,0],
...
class Gps_Signal():
    ...
    def messages(self, north, east):
        # print(north, east)
        easts = self.east_transition(east)
        norths = self.north_transition(north)
        north = abs(30.62097-float(north))
        east = abs(104.07126-float(east))
        value = max(north, east)
        # value = maximum/60
        if value<0.02:
            return ["-60~-70", norths, easts]
        if 0.02<=value<0.04:
            return ["-70~-80", norths, easts]
        if 0.04<=value<0.06:
            return ["-80~-90", norths, easts]
        if 0.06<=value<0.08:
            return  ["-90~-100", norths, easts]
        if 0.08<=value<0.1:
            return ["-100~-110", norths, easts]
    ...
    def change_northern_1(self, user, dic, carousel):
        ...
        east = value[2]
        count = value[3]
        msg = self.messages(north, east)
        print(msg)
        field_intensity = msg[0]
        norths = msg[1]
        easts = msg[2]
        send_data = "GPGLL,{},{},N,{},E,A,500,,{},<5%,-55,I,15,,62224901,,,r108.pdt.cn,108,,,,,,,".format(user,norths,easts,self.utc_time())
        if (carousel+1)%2==0:
            send_data = "GPGLL,{},{},N,{},E,A,500,,{},{},-55,I,15,,62224901,,,r124.pdt.cn,124,,,,,,,".format(user,norths,easts,self.utc_time(), field_intensity)
        a = 0
        for i in send_data:
            a = a ^ (ord(i))
